<?php
	...
	$newacct = $_POST['bankacct'];
	$newbankcode = $_POST['bankcode'];
	...
	$targetbank = $rubieslist[$key]; //this is the customers bank derived from the code array above
	
	for($i=0; $i<count($allrubies); $i++){
		
		if($allrubies[$i]['bankname'] == $targetbank)
		{
			$rubiescode = $allrubies[$i]['bankcode']; // this is their specification
			$i = count($allrubies);
		}		
	}	
	
	
	$data = array("reference" => "$reason", "amount" => "$amount", "narration" => "$reason", "craccountname" => "$recipientname", "bankname" => "$targetbank", "draccountname" => "$dname", "craccount" => "$account_number", "bankcode" => "$rubiescode");
	$data_string = json_encode($data);
	
	
	
	$curi = 'https://openapi.rubiesbank.io/v1/fundtransfer';
	$ch = curl_init();	
	...
	//echo $execResult; exit;
	
	
	$txcode = $obj->responsecode;
	$message = $obj->transactionstatus;
	$nibmsg = $obj->nibssresponsemessage;
	$resmsg = $obj->responsemessage;
	$responsedatetime = $obj->responsedatetime;
	$bankname = 'Rubies Microfinance Bank';
	
	if($txcode == '00' && strlen($txcode) == 2){
		$chkamount = $obj->amount;
		$txreference = $obj->sessionid;
		
		$outcome = "Unknown";
		
		if($message == 'Success' || $resmsg == 'Success'){
			$msg = $reference." successful |".$bankname."\n";
			$outcome = 'Success';
			$logfile = "paymentrecord.txt";
			if(file_exists($logfile)){
				$prev = file_get_contents($logfile);
				$fpc = file_put_contents($logfile, $prev.$msg);
			}else{
				$prev = "";
				$fpc = file_put_contents($logfile, $prev.$msg);
			}
		...
		$msg = $reference." successful |".$bankname."\n";				
		if(trim($resmsg) == 'Transaction is being processed'){				
			$outcome = 'Success';
			$logfile = "paymentrecord.txt";
			if(file_exists($logfile)){
				$prev = file_get_contents($logfile);
		...
		else if(trim($resmsg) == 'Unable to save transaction - please try again.' || trim($resmsg) == 'Unable to save transaction - please try again' || trim($resmsg) == 'AccountNameValidationFailed' || trim($resmsg) == 'TransactionFailed')
		{
			$outcome = 'Failed';
			$msg = $reference." failed |".$bankname."\n";
			$logfile = "paymentrecord.txt";
			if(file_exists($logfile)){
				$prev = file_get_contents($logfile);
				$fpc = file_put_contents($logfile, $prev.$msg);
		...
		else
		{
			$outcome = 'Success';
			$logfile = "paymentrecord.txt";
			$msg = $reference." successful |".$bankname."\n";//is this bank name rubies or target bank?rubies ok
			if(file_exists($logfile)){
				$prev = file_get_contents($logfile);
				$fpc = file_put_contents($logfile, $prev.$msg);
			}else{
	...
	if($outcome !== 'Success')
	{				
		$receivedata = array("reference" => "$reason");
		$data_string = json_encode($receivedata);
		
		$curi = 'https://openapi.rubiesbank.io/v1/transactionquery';
		$ch = curl_init();	
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
		curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);	
		...
		$failureReason = $obj->nibssresponsemessage;
		if($status == 'Success'){
			$outcome = "Success";
		}else{
			$outcome = "Failed";
